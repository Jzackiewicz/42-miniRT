...
/*   Updated: 2025/04/17 15:31:13 by jzackiew         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static double	*truncate_cylinder(double *arr_t, t_ray *ray, t_object *obj)
{
	double	limit;
	bool	is_t[2];
	int		i;
	...
	while (++i < 2)
	{
		if (ray->origin[1] + arr_t[i] * ray->direction[1] > 0 && ray->origin[1]
			+ arr_t[i] * ray->direction[1] < obj->height)
			is_t[i] = true;
		else
			is_t[i] = false;
	}
...
double	*truncate_caps(t_ray *ray, double *arr_t, t_object *obj)
{
	int		i;
	double	x;
	double	z;
	...
	while (++i < 2)
	{
		...
		if (pow(x, 2) + pow(z, 2) < 1 || compare_floats(pow(x, 2) + pow(z, 2),
				1))
			is_t[i] = true;
		else
			is_t[i] = false;
...
double	*intersect_caps(double *arr_t, t_ray *ray, double cyl_height)
{
	double	*out_t;
	double	limit;
	...
	// out_t = truncate_caps(ray, out_t, obj);
	// if (!out_t)
	// 	return (free(out_t), arr_t);
...
static double	*intersect_infinite_tube(t_ray *ray, double width)
{
	...
	coefficients[2] = pow(ray->origin[0], 2) + pow(ray->origin[2], 2) - width;
