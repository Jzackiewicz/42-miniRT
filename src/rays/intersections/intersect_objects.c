...
/*   Updated: 2025/04/16 20:40:02 by jzackiew         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
double	*truncate_cylinder(double *arr_t, t_ray *ray, t_object *obj)
{
	...
	while (++i < 2)
	{
		if (ray->origin[1] + arr_t[i] * ray->direction[1] > obj->coords[1] - limit &&
			ray->origin[1] + arr_t[i] * ray->direction[1] < obj->coords[1] + limit)
				is_t[i] = true;
		else
			is_t[i] = false;
	}
...
double	*truncate_caps(t_ray *ray, double *arr_t)
{
	int		i;
	double	x;
	double	z;
	...
	while (++i < 2)
	{
		...
		if (pow(x, 2) + pow(z, 2) < 1 || compare_floats(pow(x, 2) + pow(z, 2), 1))
			is_t[i] = true;
		else
			is_t[i] = false;
	}
...
double	*intersect_caps(double *arr_t, t_ray *ray, t_object *obj)
{
	double	*out_t;
	double	limit;
	
	if (compare_floats(ray->direction[1], 0) || (arr_t && !compare_floats(arr_t[0], arr_t[1])))
		return (arr_t);
	limit = obj->height / 2;
	out_t = (double *)malloc(sizeof(double) * 2);
	out_t[0] = (obj->coords[1] - limit - ray->origin[1]) / ray->direction[1];
	out_t[1] = (obj->coords[1] + limit - ray->origin[1]) / ray->direction[1];
	if (!arr_t)
		return (free(arr_t), out_t);
	out_t = truncate_caps(ray, out_t);
...
double	*intersect(t_object *obj, t_ray *ray)
{
	...
	else if (!ft_strncmp(obj->id, "cy\0", 3))
	{
		...
		arr_t = intersect_caps(arr_t, ray, obj);
	}
	else
	{
