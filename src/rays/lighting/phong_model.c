...
static double	*assign_effective_ambient(t_world *world, t_comps *comps)
{
	...
	if (!effective_ambient)
		return (NULL);
	effective_ambient[0] = world->ambient->brightness * world->ambient->color[0] / 255.0 * comps->obj->color[0];
	effective_ambient[1] = world->ambient->brightness * world->ambient->color[1] / 255.0 * comps->obj->color[1];
	effective_ambient[2] = world->ambient->brightness * world->ambient->color[2] / 255.0 * comps->obj->color[2];
		...
		t_comps *comps)
{
	...
	effective_diffuse_rgb[0] = diffuse * light_rgb[0] * world->light->brightness;
	effective_diffuse_rgb[1] = diffuse * light_rgb[1] * world->light->brightness;
	effective_diffuse_rgb[2] = diffuse * light_rgb[2] * world->light->brightness;
		...
		t_comps *comps)
{
	...
	effective_specular_rgb[0] = specular * light_rgb[0] * world->light->brightness;
	effective_specular_rgb[1] = specular * light_rgb[1] * world->light->brightness;
	effective_specular_rgb[2] = specular * light_rgb[2] * world->light->brightness;
